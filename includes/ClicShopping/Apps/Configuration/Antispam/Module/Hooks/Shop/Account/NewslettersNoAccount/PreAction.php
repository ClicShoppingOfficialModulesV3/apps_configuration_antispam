  ...
  class PreAction implements \ClicShopping\OM\Modules\HooksInterface {
    ...
    public function execute() {
      ...
      if (isset($_GET['Account']) && isset($_GET['NewslettersNoAccount']) && isset($_GET['Process'])) {
        ...
        if (defined('CLICSHOPPING_APP_ANTISPAM_AM_SIMPLE_STATUS') && CLICSHOPPING_APP_ANTISPAM_AM_SIMPLE_STATUS == 'True' && CLICSHOPPING_APP_ANTISPAM_NEWSLETTER_NO_ACCOUNT == 'True' && $error === false) {
          $error = AntispamClass::getResultSimpleAntispam();
        }

        if (defined('CLICSHOPPING_APP_ANTISPAM_RE_RECAPTCHA_STATUS') &&  CLICSHOPPING_APP_ANTISPAM_RE_RECAPTCHA_STATUS == 'True' && CLICSHOPPING_APP_ANTISPAM_NEWSLETTER_NO_ACCOUNT == 'True' && $error === false) {
          $error = $this->getResultGoogleRecaptch();
        }

        if (CLICSHOPPING_APP_ANTISPAM_INVISIBLE == 'True' && CLICSHOPPING_APP_ANTISPAM_NEWSLETTER_NO_ACCOUNT == 'True' && $error === false) {
