  ...
  class PreAction implements \ClicShopping\OM\Modules\HooksInterface {
    ...
    public function execute() {
      ...
      if (isset($_GET['Account']) && isset($_GET['CreatePro']) && isset($_GET['Process'])) {
        ...
        if ((!defined('CLICSHOPPING_APP_ANTISPAM_AM_SIMPLE_STATUS') || CLICSHOPPING_APP_ANTISPAM_AM_SIMPLE_STATUS == 'True') && (!defined('CLICSHOPPING_APP_ANTISPAM_CREATE_ACCOUNT_PRO') || CLICSHOPPING_APP_ANTISPAM_CREATE_ACCOUNT_PRO == 'False') && $error === false) {
          $error = AntispamClass::getResultSimpleAntispam();
        }

        if ((!defined('CLICSHOPPING_APP_ANTISPAM_RE_RECAPTCHA_STATUS') || CLICSHOPPING_APP_ANTISPAM_RE_RECAPTCHA_STATUS == 'True') && (!defined('CLICSHOPPING_APP_ANTISPAM_CREATE_ACCOUNT_PRO') || CLICSHOPPING_APP_ANTISPAM_CREATE_ACCOUNT_PRO == 'False') && $error === false) {
          $error = $this->getResultGoogleRecaptch();
        }

        if (defined('CLICSHOPPING_APP_ANTISPAM_INVISIBLE') && CLICSHOPPING_APP_ANTISPAM_INVISIBLE == 'True' && (!defined('CLICSHOPPING_APP_ANTISPAM_CREATE_ACCOUNT_PRO') || CLICSHOPPING_APP_ANTISPAM_CREATE_ACCOUNT_PRO == 'True')) {
        ...
        if ($error === true) {
          ...
          CLICSHOPPING::redirect('CLICSHOPPING::link(null, 'Account&CreatePro');
        }
      }
    }
  }